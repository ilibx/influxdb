...
func buildLauncherCommand(l *Launcher, cmd *cobra.Command) {
	...
	opts := []cli.Opt{
		...
		},
		{
			...
			Default: true,
			Desc:    "toggles read-only mode for the new meta store (has meaning only if the new meta store is enabled)",
		},
	}
...
func (m *Launcher) run(ctx context.Context) (err error) {
	...
	if m.enableNewMetaStore {
		...
		if m.newMetaStoreReadOnly {
			newKv, err := tenant.NewDuplicateReadTenantService(m.log, m.kvService, m.kvStore)
			if err != nil {
		...
		passwdsSvc = tenant.NewUserLogger(m.log.With(zap.String("store", "new")), tenant.NewUserMetrics(m.reg, ts, tenant.WithSuffix("pwd_new")))
	}
